...
export class Dilatation implements Filter {

  private static readonly STRUCTURING_ELEMENT = [[1, 1, 1], [1, 1, 1], [1, 1, 1]];

  private readonly _windowSize: number;

  constructor(private _se: number[][] = Dilatation.STRUCTURING_ELEMENT) {
  ...
  applyFilter(buffer: Array<Uint8Array>): Array<Uint8Array> {
    ...
    for (let y = 1; y < height-1; y++) {
      ...
      for (let x = 1; x < width-1; x++) {
        ...
        out[y][x] = foundSomething ? max : buffer[y][x];
      }
    }
